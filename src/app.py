...
def loader_for_context(path_to_file):
    """Loader for input file

    Args:
        path_to_file (str): Path of the file

    Raises:
        Exception: errors.

    Returns:
        loader: extracted data from the input documents.
    """
    print(path_to_file)
    if path_to_file.split(".")[-1] == "json":
        loader = JSONLoader(file_path=path_to_file, jq_schema=".", text_content=False)
...
def questions_from_json(file_path):
    """Returns a list of questions from the input quesrions json.
...
async def answer_questions(
    questions_file: UploadFile = File(...), context_file: UploadFile = File(...)
):
    """_summary_

    Args:
        questions_file (UploadFile, optional): JSON file with questions. Defaults to File(...).
        context_file (UploadFile, optional): JSON/PDF file with context. Defaults to File(...).

    Raises:
        Exception: error handling.

    Returns:
        json: formatted json with questions and their answers.
    """
    if model.is_valid_api_key(api_key=OPEN_API_KEY):
        ...
        if questions_file.filename.endswith(".json"):
            ...
            return {
                "error": "Unsupported file format, input file should be in json format"
            }
        ...
        if context_file.filename.endswith(".json"):
            loader = JSONLoader(
                file_path=context_file_path, jq_schema=".", text_content=False
            )
        ...
        # Iterate over given questions.
        answers = []
        for index, question in enumerate(questions):
            answer = model.answer_questions(
                question, number_of_documents_to_review=5, temperature=0
            )
            answer = {
                "question": question,
